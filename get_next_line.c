...
/*   Updated: 2024/03/07 11:12:17 by aoulahra         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "get_next_line.h"

char	*ft_substr(char *buffer, size_t start, size_t end, int if_free)
{
	size_t	i;
	char	*pol_line;
...
char	*ft_get_line(int fd, int *fn)
{
	...
	while (!(*fn))
	{
		...
		if (bytes <= 0)
			break ;
		tmp_buffer[bytes] = '\0';
		next_line = ft_strjoin(next_line, tmp_buffer, fn, 1);
		if (!next_line)
...
char	*get_next_line(int fd)
{
	...
	if (!line && ((buffer && buffer[0] == '\0') || fn == -1))
		return (buffer = ft_freeit(buffer, NULL));
	tmp_buffer = ft_strjoin(buffer, line, &fn, 2);
	if (!tmp_buffer)
		return (buffer = ft_freeit(NULL, line));
	line = ft_substr(tmp_buffer, 0, ft_strlen_n(tmp_buffer, &fn, 1), 0);
	if (!line)
		return (buffer = ft_freeit(tmp_buffer, NULL));
	buffer = ft_substr(tmp_buffer, ft_strlen_n(tmp_buffer, &fn, 1),
